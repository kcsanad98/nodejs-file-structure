...
function handleDepthInput(input) {
    ...
    if (depthInput === false) {
        console.log("Depth must be a positive integer. Please provide a valid value:");
    } else {
        depth = depthInput;
    }
}
...
function handleDirInput(input) {
    if (isDirValid(input)) {
        dir = input;
        console.log("Depth of search (if not specified the largest possible depth will be used):");
    } else {
